...
exports.initGame = function(sio, socket){
    serverSocket = sio;
    gamersSocket = socket;
    gamersSocket.emit('connected', { message: "You are connected!" });

    // Host Events
    gamersSocket.on('hostCreateNewGame', hostCreateNewGame);
...
function hostNextRound(data) {
    //TODO hier checken was vom host kam. Spiel vorbei? -> emit('gameover')
    if(data.round < wordPool.length ){
        // Send a new set of words back to the host and players.
        sendWord(data.round, data.gameId);
    } else {
        // If the current round exceeds the number of words, send the 'gameOver' event.
        serverSocket.sockets.in(data.gameId).emit('gameOver',data);
    }
}*/
...
function playerAnswer(data) {
    ...
    //TODO hier positionsDaten von den Baellen verschicken und im host verarbeiten
    // (er muss selber die bricks etc wegmachen. akutelle Punkte mitschicken oder ebenfalls vom host berechnen lassen?)
    serverSocket.sockets.in(data.gameId).emit('hostCheckAnswer', data);
}
...
function sendWord(wordPoolIndex, gameId) {
    ...
    serverSocket.sockets.in(data.gameId).emit('newWordData', data);
}*/
